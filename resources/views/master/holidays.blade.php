...
<div class="space-y-6">
    ...
    <div class="p-4 sm:p-6 rounded-2xl bg-gradient-to-r from-rose-600 via-red-600 to-amber-600 text-white shadow-lg shadow-rose-600/15 relative overflow-hidden">
        ...
        <div class="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4 sm:gap-6">
            <div>
                <div class="inline-flex items-center gap-2 px-2.5 py-0.5 rounded-full bg-white/15 backdrop-blur-md text-white text-[11px] font-semibold mb-2">
                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/>
                    </svg>
                    Master Data Kalender & Tanggal Merah
                </div>
                <h1 class="text-lg sm:text-xl font-extrabold tracking-tight">Hari Libur & Tanggal Merah</h1>
                <p class="text-xs text-rose-100 mt-0.5 max-w-2xl leading-relaxed">
                    ...
                    <span class="font-bold underline decoration-white/40">Sistem presensi otomatis nonaktif</span> pada tanggal yang didaftarkan.
                </p>
            </div>
            <div class="flex flex-wrap items-center gap-2">
                <button type="button" onclick="openAddModal()"
                    class="px-3.5 py-2 bg-white text-rose-700 hover:bg-rose-50 font-bold rounded-xl text-xs flex items-center gap-1.5 shadow-md shadow-black/10 transition transform active:scale-95 cursor-pointer">
                ...
                <form action="{{ route('master.holidays.generate_common') }}" method="POST" onsubmit="return confirm('Muat otomatis daftar Hari Libur Nasional & Kalender Pendidikan tahun {{ $year }}?')">
                    ...
                    <button type="submit"
                        class="px-3.5 py-2 bg-white/20 hover:bg-white/30 text-white font-bold rounded-xl text-xs flex items-center gap-1.5 backdrop-blur-md transition transform active:scale-95 cursor-pointer"
                ...
                <button type="button" onclick="openImportModal()"
                    class="px-3.5 py-2 bg-white/20 hover:bg-white/30 text-white font-bold rounded-xl text-xs flex items-center gap-1.5 backdrop-blur-md transition cursor-pointer">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <!-- Stat Cards (Refined & Compact) -->
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div class="p-3.5 rounded-2xl bg-white border border-slate-200 shadow-2xs">
            <div class="flex items-center justify-between">
                <span class="text-[11px] font-semibold text-slate-500">Total Terdaftar ({{ $year }})</span>
                <span class="w-6 h-6 rounded-lg bg-slate-100 text-slate-600 flex items-center justify-center text-[10px] font-bold">📅</span>
            </div>
            <div class="text-xl font-black text-slate-900 mt-1.5">{{ $stats['total'] }}</div>
            <p class="text-[10px] text-slate-400 mt-0.5">Hari / agenda libur</p>
        </div>
        <div class="p-3.5 rounded-2xl bg-white border border-slate-200 shadow-2xs">
            <div class="flex items-center justify-between">
                <span class="text-[11px] font-semibold text-slate-500">Libur Nasional</span>
                <span class="w-6 h-6 rounded-lg bg-rose-50 text-rose-600 flex items-center justify-center text-[10px] font-bold">🔴</span>
            </div>
            <div class="text-xl font-black text-rose-600 mt-1.5">{{ $stats['nasional'] }}</div>
            <p class="text-[10px] text-slate-400 mt-0.5">Tanggal merah resmi</p>
        </div>
        <div class="p-3.5 rounded-2xl bg-white border border-slate-200 shadow-2xs">
            <div class="flex items-center justify-between">
                <span class="text-[11px] font-semibold text-slate-500">Cuti Bersama / Khusus</span>
                <span class="w-6 h-6 rounded-lg bg-amber-50 text-amber-600 flex items-center justify-center text-[10px] font-bold">🟡</span>
            </div>
            <div class="text-xl font-black text-amber-600 mt-1.5">{{ $stats['khusus'] }}</div>
            <p class="text-[10px] text-slate-400 mt-0.5">Keagamaan / khusus</p>
        </div>
        <div class="p-3.5 rounded-2xl bg-white border border-slate-200 shadow-2xs">
            <div class="flex items-center justify-between">
                <span class="text-[11px] font-semibold text-slate-500">Libur Sekolah</span>
                <span class="w-6 h-6 rounded-lg bg-indigo-50 text-indigo-600 flex items-center justify-center text-[10px] font-bold">🔵</span>
            </div>
            <div class="text-xl font-black text-indigo-600 mt-1.5">{{ $stats['sekolah'] }}</div>
            <p class="text-[10px] text-slate-400 mt-0.5">Semester & KBM off</p>
        </div>
    </div>
